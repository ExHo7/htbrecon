...
main() {
        ...
        if [ -n "$FOUND_SUBDOMAINS" ]; then
            ...
            while read -r sub; do
                ...
                if [ "$RECURSION" = true ]; then
                    ...
                    nuclei -u "http://$full_subdomain" -as -json-export "$NAME/nuclei_${sub}.json" > "$NAME/nuclei_${sub}.log" 2>&1
                    BG_PIDS+=($!)
                    ...
                    ffuf -u "http://$full_subdomain/FUZZ" -w "$WORDLIST" -t $THREADS -recursion-depth $DEPTH -e $EXTENSIONS -ac -o "$NAME/dirscan_${sub}.json" > "$NAME/ffuf_${sub}.log" 2>&1
                    BG_PIDS+=($!)
    ...
    ffuf -u "${URL%/}/FUZZ" -w "$WORDLIST" -t $THREADS -recursion-depth $DEPTH -e $EXTENSIONS -ac -o "$NAME/dirscan.json" > /dev/null 2>&1
    FFUF_EXIT=$?
    if [ $FFUF_EXIT -eq 0 ] || [ $FFUF_EXIT -eq 2 ]; then
